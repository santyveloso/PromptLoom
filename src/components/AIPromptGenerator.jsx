...
export default function AIPromptGenerator({ onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const geminiClient = new GeminiClient(import.meta.env.VITE_GEMINI_API_KEY);
  ...
  const handleClose = () => {
    if (stage === "complete") {
      ...
      // Show confirmation if processing
        ...
        <div className="p-6">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-xl font-semibold text-gray-900">
              {stage === "input" && "✨ AI Fill - Describe Your Prompt"}
              {stage === "clarification" && "🤔 A Few Quick Questions"}
              {stage === "processing" && "⚙️ Generating Your Prompt"}
              {stage === "complete" && "✅ Prompt Generated Successfully"}
            </h3>
            <button
              onClick={handleClose}
